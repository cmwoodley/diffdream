...
class UNet3D_tcond(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, level_channels=[64, 128, 256], bottleneck_channel=512, num_timesteps=1000) -> None:
        super(UNet3D_tcond, self).__init__()
        level_1_chnls, level_2_chnls, level_3_chnls = level_channels[0], level_channels[1], level_channels[2]
        self.embed_timesteps = nn.Embedding(num_timesteps, in_channels)
        self.a_block1 = Conv3DBlock(in_channels=in_channels+5, out_channels=level_1_chnls)
        self.a_block2 = Conv3DBlock(in_channels=level_1_chnls, out_channels=level_2_chnls)
        self.a_block3 = Conv3DBlock(in_channels=level_2_chnls, out_channels=level_3_chnls)
        self.bottleNeck = Conv3DBlock(in_channels=level_3_chnls, out_channels=bottleneck_channel, bottleneck= True)
        self.s_block3 = UpConv3DBlock(in_channels=bottleneck_channel, res_channels=level_3_chnls)
    ...
    def forward(self, input, timesteps):
        ...
        emb = self.embed_timesteps(timesteps)

        bs, ch, d, w, h = input.shape

        emb = emb.view(bs, emb.shape[1],1,1,1).repeat(1,1,d,w,h)

        input = torch.cat([emb,input], dim=1)
...
class UNet3D_cond(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, level_channels=[64, 128, 256], bottleneck_channel=512) -> None:
        super(UNet3D_cond, self).__init__()
        level_1_chnls, level_2_chnls, level_3_chnls = level_channels[0], level_channels[1], level_channels[2]
        self.a_block1 = Conv3DBlock(in_channels=in_channels+3, out_channels=level_1_chnls)
        self.a_block2 = Conv3DBlock(in_channels=level_1_chnls, out_channels=level_2_chnls)
        self.a_block3 = Conv3DBlock(in_channels=level_2_chnls, out_channels=level_3_chnls)
        self.bottleNeck = Conv3DBlock(in_channels=level_3_chnls, out_channels=bottleneck_channel, bottleneck= True)
        self.s_block3 = UpConv3DBlock(in_channels=bottleneck_channel, res_channels=level_3_chnls)
    ...
    def forward(self, input, pharm):
        #Analysis path forward feed

        input = torch.cat([pharm,input], dim=1)
...
class UNet3D_ptcond(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, level_channels=[64, 128, 256], bottleneck_channel=512, num_timesteps=1000) -> None:
        super(UNet3D_ptcond, self).__init__()
        level_1_chnls, level_2_chnls, level_3_chnls = level_channels[0], level_channels[1], level_channels[2]
        self.embed_timesteps = nn.Embedding(num_timesteps, in_channels)
        self.embed_pharm = nn.Sequential(
          nn.Conv3d(3,1,(3,3,3),padding=1),
          nn.ReLU(),
          nn.Linear(24,24)
        )
        self.a_block1 = Conv3DBlock(in_channels=in_channels+6, out_channels=level_1_chnls)
        self.a_block2 = Conv3DBlock(in_channels=level_1_chnls, out_channels=level_2_chnls)
        self.a_block3 = Conv3DBlock(in_channels=level_2_chnls, out_channels=level_3_chnls)
        self.bottleNeck = Conv3DBlock(in_channels=level_3_chnls, out_channels=bottleneck_channel, bottleneck= True)
        self.s_block3 = UpConv3DBlock(in_channels=bottleneck_channel, res_channels=level_3_chnls)
    ...
    def forward(self, input, timesteps, pharm):
        ...
        pemb = self.embed_pharm(pharm)

        input = torch.cat([emb,pemb,input], dim=1)
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.conv3d1 = nn.Conv3d(in_channels=5, out_channels=8, kernel_size=(3,3,3), stride=(1,1,1),padding=1)
