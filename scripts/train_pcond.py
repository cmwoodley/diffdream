...
from networks import EncoderCNN, DecoderRNN, UNet3D_ptcond, Encoder
...
log_file = open(os.path.join("../reports/log_pcond.txt"), "w")
...
if args.restart == True:
    ...
    if args.prev <= 6000:
        net.load_state_dict(torch.load("../models/net_pharm_weights_"+str(args.prev)+".pkl"))
        smiles = smiles[args.prev*batch_size:]
        start_i = args.prev
    else:
        net.load_state_dict(torch.load("../models/net_pharm_weights_"+str(args.prev)+".pkl"))
        encoderCNN.load_state_dict(torch.load("../models/encoderCNN_pharm_weights_"+str(args.prev)+".pkl"))
        decoderRNN.load_state_dict(torch.load("../models/decoderRNN_pharm_weights_"+str(args.prev)+".pkl"))
        smiles = smiles[args.prev*batch_size:]
        start_i = args.prev    
...
for i, (x, captions, pharm, lengths) in enumerate(train_dataloader):
    ...
    pred = net(noisy_x, timesteps.to(device), pharm.to(device))
    net_loss = criterionNet(pred, x.to(device))
    ...
    if i+start_i > 6000:
        ...
        noisy_x = scheduler.add_noise(x.to(device), noise.to(device), torch.stack([scheduler.timesteps[999]]*x.shape[0]).to(device))    
        pred = net(noisy_x.to(device),torch.stack([scheduler.timesteps[999]]*x.shape[0]).to(device), pharm.to(device)).detach()
    ...
    if (i+start_i+1) % 500 == 0:
        if (i+start_i) <= 6000:
            if (i+start_i+1)%4000 == 0:
                torch.save(net.state_dict(),"../models/net_pharm_weights_{}.pkl".format(i+start_i+1))

            log_file.write("Batch: {}\nNet Loss: {}\n".format((i+start_i+1),net_loss))
            log_file.flush()

        else:
            if (i+start_i+1)%4000 == 0:
                torch.save(net.state_dict(),"../models/net_pharm_weights_{}.pkl".format(i+start_i+1))
                torch.save(encoderCNN.state_dict(),"../models/encoderCNN_pharm_weights_{}.pkl".format(i+start_i+1))
                torch.save(decoderRNN.state_dict(),"../models/decoderRNN_pharm_weights_{}.pkl".format(i+start_i+1))
            log_file.write("Batch: {}\nNet Loss: {}\nCaptioning Loss: {}.\n".format((i+start_i+1),net_loss,cap_loss))
            log_file.flush()
